...
def run_signalaggregation_strategy(df: pd.DataFrame, vote_threshold: int = 5, start_kapital: float = 100000):
...
def run_for_period(start_date, end_date, output_path):
    ...
    for ticker in tickers_to_test:
        ...
        for vt in vote_thresholds:
            try:
                fig1, fig2, final_value, _ = run_signalaggregation_strategy(df, vote_threshold=vt, start_kapital=start_kapital)
...
def main():
    ...
    for label, (start_date, end_date) in date_periods.items():
        output_file = f"results_signalaggregation_{label}.txt"
